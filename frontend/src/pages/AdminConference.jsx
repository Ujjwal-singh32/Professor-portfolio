...
const AdminPanelConference = () => {
  ...
  const { conference, backendUrl } = useContext(UserContext);
  //console.log(achieve);
    ...
    try {
      if (editDoc) {
        const response = await axios.put(
          `${backendUrl}/api/conference/update-Conferences/${newDoc._id}`,
          {name : newDoc.name , description : newDoc.description ,date:newDoc.date }
        );

        if (response.data.success) {
          toast.success("Conferences Updated");
          navigate("/");
        }
      } else {
        ...
        const response = await axios.post(
          `${backendUrl}/api/conference/send-Conference`,
  ...
  const handleDelete = async (id) => {
    const response = await axios.delete(
      `${backendUrl}/api/conference/delete/${id}`
    );
    if (response.data.success) {
...
export default AdminPanelConference;
