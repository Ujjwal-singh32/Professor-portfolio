...
const AdminPanelAward = () => {
  ...
  const { award, backendUrl } = useContext(UserContext);
  //console.log(achieve);
    ...
    try {
      if (editDoc) {
        const response = await axios.put(
          `${backendUrl}/api/award/update-award/${newDoc._id}`,
          {name : newDoc.name , description : newDoc.description ,year:newDoc.year }
        );
        ...
        const response = await axios.post(
          `${backendUrl}/api/award/send-award`,
          {name : newDoc.name , description : newDoc.description , year : newDoc.year }
        );
        if (response.data.success) {
  ...
  const handleDelete = async (id) => {
    const response = await axios.delete(
      `${backendUrl}/api/award/delete/${id}`
    );
    if (response.data.success) {
  ...
  return (
    ...
    >
      <div className="p-12 shadow-lg rounded-lg text-center w-full max-w-4xl text-white">
        <h1 className="text-7xl font-extrabold text-white mb-6">Award Admin Panel</h1>
        ...
        {(action === "add" || editDoc) && (
          <div className="mt-4 p-6 bg-gray-500 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-white mb-4">
              {editDoc ? "Edit Award" : "Add a New Award"}
            </h2>
            <input
              name="name"
              value={newDoc.name}
            ...
            >
              {editDoc ? "Update Award" : "Save Award"}
            </button>
          </div>
        )}
...
export default AdminPanelAward;
